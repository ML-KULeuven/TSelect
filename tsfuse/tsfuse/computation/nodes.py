...
class Transformer(Node):
    ...
    def check_preconditions(self, *collections, return_bool=True):
        ...
        *collections
        ...
        return_bool : bool, optional (default=False)
            If True, return True if the preconditions are satisfied and False otherwise instead of raising an error.

        Returns
        -------
        satisfied : bool
            True if the preconditions are satisfied and False otherwise. Only returned if ``return_bool=True``, otherwise
            an error is raised if the preconditions are not satisfied.
        """

        def satisfied(c):
            precon = all(p(*c) for p in self.preconditions)
            precon_parents = all(p(*self.parents) for p in self.preconditions_parents)
            return precon and precon_parents
    ...
    def transform(self, *collections, ignore_preconditions=False):
        if not ignore_preconditions:
            self.check_preconditions(*collections, return_bool=False)
        result = None
...
def _is_parameter(p):
    if p[0].startswith('_'):
        ...
        return p not in ('preconditions', 'preconditions_parents', )
